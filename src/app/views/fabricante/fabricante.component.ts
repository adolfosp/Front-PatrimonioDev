...
export class FabricanteComponent extends Componente implements OnInit {
  ...
  public salvarAlteracao(): void {
    ...
    this._fabricante = (this.estadoSalvar === 'cadastrarFabricante') ? { ...this.form.value } : { codigoFabricante: this._fabricante.codigoFabricante, ...this.form.value };

    this.fabricanteService[this.estadoSalvar](this._fabricante).subscribe(
      () => this.mostrarAvisoSucesso(`Fabricante ${nomeAcaoRealizada} com sucesso`),
      (error: unknown) => {
        this.mostrarAvisoErro(error, `${MensagemRequisicao.retornarMensagemDeErroAoRealizarOperacao(nomeAcaoRealizada, "fabricante", ['o', 'do'])}`);
      },
      () => {
  ...
  public carregarFabricante(): void {
    ...
    if (this._codigoFabricante !== null && this._codigoFabricante !== 0) {
      ...
      this.fabricanteService.obterApenasUmFabricante(this._codigoFabricante).subscribe(
        {
