...
export class PatrimonioComponent extends Componente implements OnInit {
	...
	private _paginacaoSelectEquipamento: Pagination;
    ...
    public readonly metodoCarregarEquipamento = "obterEquipamentos";
	...
	constructor(
		...
		super();
		this.chaveSituacaoEquipamento = Object.keys(
			this.situacaoEquipamentoEnum,
		).filter(Number);

		this._paginacaoSelectEquipamento = configuracaoPaginacao;
	}
	...
	public salvarAlteracao(): void {
		...
		this.patrimonio.situacaoEquipamento =
			+this.form.controls["situacaoEquipamento"].value;
			...
			this.estadoSalvar === "cadastrarPatrimonio"
				...
				: {
						codigoInformacaoAdicional:
							this.informacaoAdicional.codigoInformacaoAdicional,
						...this.formAdicional.value,
				  };
		this.patrimonioService[this.estadoSalvar](
			this.patrimonio,
			this.informacaoAdicional,
		)
			.subscribe(
				() =>
					this.mostrarAvisoSucesso(
						`Patrimônio ${nomeAcaoRealizada} com sucesso`,
					),
				(error: unknown) => {
	...
	public carregarPatrimonio(): void {
		this.activatedRoute.queryParams.subscribe((parametro) => {
			(this.codigoPatrimonio = parametro["codigoPatrimonio"]),
				(this.serviceTag = parametro["serviceTag"]);
		});

		if (
			this.codigoPatrimonio !== null &&
			this.codigoPatrimonio !== 0 &&
			typeof this.codigoPatrimonio != "undefined"
		) {
			...
			this.patrimonioService
				...
				.subscribe(
					(listaDeResposta) => {
						...
						this.valorAtualSituacaoEquipamento =
							listaDeResposta[0].situacaoEquipamento.toString();
					},
					(error: unknown) => {
						this.mostrarAvisoErro(
							error,
							"Houve um erro ao tentar carregar o patrimônio.",
						);
					},
				)
	...
	onSelectAberto(event: any, select: string, nomeMetodo: string) {
		...
		this[select].panel.nativeElement.addEventListener("scroll", () => {
			...
			const paginacao = this.selectService.ObterPaginacao(
				this._paginacaoSelectEquipamento,
			);
	...
	private obterEquipamentos(paginacaoBase: Paginacao = null): void {
        ...
        let paginacaoEquipamento = new Paginacao(this._paginacaoSelectEquipamento.offset, this._paginacaoSelectEquipamento.limit);

        if (paginacaoBase !== null) paginacaoEquipamento = paginacaoBase;

		this.equipamento.obterTodosEquipamentos(paginacaoEquipamento).subscribe({
			...
			error: (error: unknown) => {
				this.mostrarAvisoErro(
					error,
					"Houve um problema ao carregar os equipamentos.",
				);
			},
		});
	}
	...
	private validarCamposPatrimonio(): void {
		this.form = this.fb.group({
			codigoPatrimonio: new FormControl(
				this.limpandoCampo ? this.form.get("codigoPatrimonio").value : 0,
				[],
			),
	...
	private validarCamposInformacaoAdicional(): void {
		this.formAdicional = this.fbe.group({
			codigoInformacaoAdicional: new FormControl(
				this.limpandoCampo
					? this.formAdicional.get("codigoInformacaoAdicional").value
					: 0,
				[],
			),
