...
var FuncionarioComponent = /** @class */ (function () {
    ...
    FuncionarioComponent.prototype.validacao = function () {
        this.form = this.fb.group({
            codigoFuncionario: new forms_1.FormControl(this._limpandoCampo ? this.form.get('codigoFuncionario').value : 0, []),
    ...
    FuncionarioComponent.prototype.salvarAlteracao = function () {
        ...
        this._funcionario = (this.estadoSalvar === 'cadastrarFuncionario') ? __assign({}, this.form.value) : __assign({ codigoFuncionario: this._funcionario.codigoFuncionario }, this.form.value);
        this.funcionarioService[this.estadoSalvar](this._funcionario).subscribe(function () { return _this.toaster.success("Funcion\u00E1rio " + nomeAcaoRealizada + " com sucesso", 'Sucesso!'); }, function (error) {
        ...
        if (this._codigoFuncionario !== null && this._codigoFuncionario !== 0) {
            ...
            this.funcionarioService.obterApenasUmFuncionario(this._codigoFuncionario).subscribe({
                next: function (funcionario) {
                    _this._funcionario = __assign({}, funcionario);
                    _this.form.patchValue(_this._funcionario);
                },
                error: function (error) {
                    var template = MensagemRequisicaoHelper_1.MensagemRequisicao.retornarMensagemTratada(error["message"], error["error"].mensagem);
